...
public class SlipLink extends Link {
	...
	public static SlipLink createSlipLink(String name, int intrinLen,
			int maxShrunkLen, SlipnetNode from, SlipnetNode to,
			SlipnetNode label) {
	...
	private int getCurrentLength() {
		...
		if (activation > activationThreshold) {
			...
			double temp = 1.0 - ((activationD - activationThresholdD) / (maxActivationD - activationThresholdD));
