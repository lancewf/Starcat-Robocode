...
public class SlipnetNode 
implements Cloneable, Keyable, StarcatObject
{
// -----------------------------------------------------------------------------
// #region Private Data
// -----------------------------------------------------------------------------   
   ...
   private Object key;
	...
	private List<Codelet> codeletList;
	private int conceptualDepth;   

	//instrumentation
	private int numActivationIncreaseAttempts;
	
	/*
	 * When the SlipnetNode goes to full activation, this is the number 
	 * of Codelet objects posted.
	 */
	private int numCodeletsToPost;
	private int numTimesFullyActivated;
	
	/*
	 * Sets the number of slipnet updates that occur before activation 
	 * can change.  Default is 0.
	 */
	private int numUpdatesToClampAct = 0;
	
	private Slipnet slipnet;
	
	private PublicStarcatObject sObjDelegate =
...
// -----------------------------------------------------------------------------
	...
	public SlipnetNode()
	{
		codeletList = new ArrayList<Codelet>();
		this.numActivationIncreaseAttempts = 0;
		this.numTimesFullyActivated = 0;
	}
	
	public SlipnetNode(String name, int conceptualDepth,
			int activation, int activationThreshold,
			int numCodeletsToPost) 
	{
		this();
		if (name == null)
		{
...
// -----------------------------------------------------------------------------
   ...
   public LateralLink getIdentityLink()
   {
	...
	/*
	 * a little funky, this takes the other node and looks to see if it 
	 * has a property link to this node, since property links are not 
	 * really bidirectional
	 */
    ...
    * the longer memory is stored in the node.
	...
	public synchronized void spreadActivation() 
   {
	...
	/*
	 * Algorithm:
	 * if node is not at full activation
	 *    if activation + buffer is at least max activation
	 *       then go to full activation
	 *       else add buffer to current activation
	 * if node is above threshold
	 *    probabilistically go to full activation
	 *       if at full activation
	 *          then probabilistically post codelets
	 * decay()
	 */
	public void update() 
	{
...
// -----------------------------------------------------------------------------
// #region Implemented members of Keyable
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
   ...
   /**
    * Post codelets to the coderack with the given probability
    * <b>
    * Up to the given number of codelets to post, probabilistically 
    * post the codelets to the coderack based on probability
    * 
    * @param probability the probability to post each new codelet
    */
   ...
   /*
    * Algorithm:
    * Activation buffer goes to 0
    * if (node has activation above min)
    *    then reduce activation by amount equal to 1% of difference 
    *    between max activation and conceptual depth times the 
    *    current activation
    *    if (the difference between the current activation and the 
    *    amount to be reduced puts the node at minimum activation or 
    *    below)
    *       then set the node's activation to minimum
    *    else reduce activation by calculated amount
    *
    */
   private void decay()
   {
      ...
      {
         int ammount = (int) Math.ceil
         (0.01*(MAXACTIVATION - conceptualDepth)*activation);
   ...
   private boolean isLinkedTo(List<? extends Link> linkList, SlipnetNode anotherNode)
   {
      if (linkList == null)
      {
         return false;
   ...
   /**
    * The algorithm for the probability of posting codelets is basically 
    * the amount over the threshold the SlipnetNode is 
    * divided by the maximum activation less the activation threshold.
    */
